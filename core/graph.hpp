...
public:
  ...
  VertexId vertices;
  EdgeId edges;
  int hot_piece; //the value decide piece hot or not
  
  #ifdef IF_PAGERANK
  double * piece_statedegree; // every 64 vertices's delta summury
  #else
  long long * piece_statedegree; // every 64 vertices's delta summury
  #endif

  VertexSubset * active; //initial processed vertices (hot piece)
...
//todo
    ...
    #ifdef IF_PAGERANK
    piece_statedegree = new double [(vertices >> 6) +1];
    #pragma omp parallel for
    for (size_t i=0;i<(vertices >> 6)+1;i++) {
      piece_statedegree[i] = 0;
    }
    active = new VertexSubset(vertices);
    active->clear();
    int count = 0;

    hot_piece = 1200;//50 100 150 200 250 300 350 400
    for(VertexId vtx = 0; vtx < vertices; vtx++){
       // piece_statedegree[vtx >> 6] += 0.75 * out_degree[vtx] - 0.25 * in_degree[vtx];
        piece_statedegree[vtx >> 6] += out_degree[vtx] + 0.5 * in_degree[vtx];
        if(((vtx >> 6) < ((vtx+1) >> 6)) && (piece_statedegree[vtx >> 6] > hot_piece)){
          // printf("hot piece[%d] =%lf\n",vtx >> 6, piece_statedegree[vtx >> 6]);
           active->set_piece_bit(vtx >> 6);
           count += 1;
        }
    }

    #else
    piece_statedegree = new long long [(vertices >> 6) +1];
    #pragma omp parallel for
    for (size_t i=0;i<(vertices >> 6)+1;i++) {
      piece_statedegree[i] = 0;
    }
    ...
    hot_piece = 2;
    for(VertexId vtx = 0; vtx < vertices; vtx++){
        piece_statedegree[vtx >> 6] += out_degree[vtx];
        if(((vtx >> 6) < ((vtx+1) >> 6)) && ((piece_statedegree[vtx >> 6]/64) > hot_piece)){
           active->set_piece_bit(vtx >> 6);
        }
    }
    #endif
    printf("hot piece:%lf\n",(double)count/((vertices >> 6) +1));
//    for(int i=0;i<(vertices >> 6)+1;i++) printf("%lu\n",active->data[i]);
//todo:end
...
//    }
    ...
    for (int t_i=0;t_i<threads;t_i++) {
      int s_i = get_socket_id(t_i);
      int s_j = get_socket_offset(t_i);
      //printf("socket_id s_i:%d socket_offset s_j:%d\n",s_i,s_j);
      ...
      //printf("thread_state[%d]->curr:%d thread_state[%d]->end:%d\n",t_i,thread_state[t_i]->curr,t_i,thread_state[t_i]->end);
        ...
        while (word != 0) {
          if (word & 1) {
            if(std::is_same<R,double>::value){
              R temp_process = process(v_i);
              piece_statedegree[(v_i) >> 6] += temp_process;
              local_reducer += temp_process;
            }
            else local_reducer += process(v_i);
          }
          v_i++;
          word = word >> 1;
        }
    ...
    #ifdef PRINT_DEBUG_MESSAGES
    printf("every 64 vertices state degree:\n");
    for(size_t i=0;i<(vertices >> 6)+1;i++){
      printf("process_vertices:%lf\n",piece_statedegree[i]);
    }                
    #endif
    R global_reducer;
  ...
  R process_edges(std::function<void(VertexId)> sparse_signal, std::function<R(VertexId, M, VertexAdjList<EdgeData>)> sparse_slot, std::function<void(VertexId, VertexAdjList<EdgeData>)> dense_signal, std::function<R(VertexId, M)> dense_slot, Bitmap * active, Bitmap * dense_selective = nullptr) {
    ...
    stream_time -= MPI_Wtime();
//todo:
    VertexId sparse_vertices_active = 0;
//todo:end
    for (int t_i=0;t_i<threads;t_i++) {
...
//todo:   
    sparse = false; 
   // printf("active_edges:%d edges/20:%d\n",active_edges,edges/20);
//todo:end
    if (sparse) {
      for (int i=0;i<partitions;i++) {
        for (int s_i=0;s_i<sockets;s_i++) {
    ...
    if (sparse) {
      ...
      for (VertexId begin_v_i=partition_offset[partition_id];begin_v_i<partition_offset[partition_id+1];begin_v_i+=basic_chunk) {
        ...
        while (word != 0) {
          if (word & 1) {
            sparse_signal(v_i);
            __sync_fetch_and_add(&sparse_vertices_active,1);
          }
          v_i++;
          word = word >> 1;
//          __sync_fetch_and_add(&vertices_active,1);
        }
      }
      #pragma omp parallel for
      for (int t_i=0;t_i<threads;t_i++) {
      ...
      delete [] recv_queue;
//todo:
      printf("under sparse active vertices:%d\n",sparse_vertices_active);
//todo:end      
    } else {
